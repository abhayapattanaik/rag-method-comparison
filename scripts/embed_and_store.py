"""embed_and_store.py — Embed all chunks and upsert into ChromaDB.

Idempotent: skips embedding if the target collection already has the expected
chunk count. Loads chunks from data/cache/chunks.json (produced by the
chunking phase).
...
CHUNKS_JSON_PATH = _PROJECT_ROOT / "data" / "cache" / "chunks.json"
...
def _load_chunks(path: Path) -> list[Chunk]:
    """Deserialise chunks from the JSON cache produced by the chunking script."""
...
def main() -> None:
    ...
    if not CHUNKS_JSON_PATH.exists():
        print(f"ERROR: chunks.json not found at {CHUNKS_JSON_PATH}")
        print("Run the chunking script first: scripts/chunk_papers.py")
        sys.exit(1)
